...
class FormProductosCubit extends Cubit<FormProductosState> {
  ...
  selectEmpresa(Empresa empresa,int index) => emit(state.copyWith(empresa: empresa,empresaSelecionada: index));
  
  selectCategoria(CategoriaProducto categoria,int index) => emit(state.copyWith(categoria: categoria,categoriaSelecionada: index));
}
